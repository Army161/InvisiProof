...
// Captures console.log/warn/error and sends to dev server for debugging
      ...
      fetch(url, {
        ...
        if (!fetchErrorLogged) {
          ...
          if (typeof window !== 'undefined' && window.console) {
            (window.console as any).__proto__.log.call(console, '[ProofLoop] Fetch error (will not repeat):', e.message || e);
          }
        }
      });
    } catch (e) {
...
export const setupErrorLogging = () => {
  ...
  originalConsoleLog('[ProofLoop] Setting up error logging...');
  originalConsoleLog('[ProofLoop] Log server URL:', logServerUrl || 'NOT AVAILABLE');
  originalConsoleLog('[ProofLoop] Platform:', Platform.OS);
